...
WHERE students.first_name || ' ' || students.last_name = 'Lea Benichou';
...
VALUES (	'Omer', 
		  	'Simpson', 
		  	(SELECT students.birth_date FROM students WHERE students.id = 4),
		  	70
		  );
...
SELECT students.first_name, 
		 students.last_name, COUNT(students.math_grade) AS total_grade
FROM students
GROUP BY students.first_name, students.last_name;

-- SUM
-- 1) Find the sum of all the students grades.
SELECT SUM(students.math_grade) AS total_students_grade
FROM students;
